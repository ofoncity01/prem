...
#
def banner():
	os.system('echo  "\n\n╔═══════════════════════════════════════════╗\n║  #TOOL-AUTHOR  : Mark Cornel              ║\n║  #Github       : GUPTA-SHAKEL             ║ \n║  #WatsApp      : +2347013107449           ║ \n╚═══════════════════════════════════════════╝\n    \n╔═════════════════════════════════════════╗\n║  #NOTE       : FREE USERS STAY OFF      ║\n║  #STATUS     : PREMIUM                  ║ \n║  #USER LEVEL : PAID                     ║\n╚═════════════════════════════════════════╝" | lolcat -a -d 2 -s 50')
# Logo
...
def chk():
  ...
  try: 
    ...
    if id in httpCaht: 
      ...
      print("\033[0;93m YOUR ID IS NOT ACTIVE COPY👆 AND SEND ME MESSAGE ON WHATSAPP \033[0;91m#NOT FREE!!!") 
      os.system('xdg-open  https://wa.me/2347045245622?text=Dear%20Admin,%20Please%20Approved%20My%20ID%20To%20Premium%20')
      time.sleep(1) 
      sys.exit() 
  except: 
    sys.exit() 
...
def ___follow___():
    try:
        ...
        ___teks = random.choice(['Hallo Bang 😍', 'Hi Bang, how are you ', 'Permission to use the script ', 'Thats great ', 'Programmer Bang ', 'Greetings to know Bang ','I Love You ❤️'])
...
class ___crack___:
    ...
    def __init__(self):
        ...
        try:
            print(f"\n{H}[{P}•{H}]{P} Ok Results Saved In Results/Ok.txt")
            print(f"{H}[{P}•{H}]{P} Cp Results Saved In Results/Cp.txt\n")
